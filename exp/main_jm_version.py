...
def process_data(n_classes_dep, n_ind, n_categorical, n_classes_ind, n_samp):
    """Generates and processes data for a given configuration."""
    data_generator = DataGenerator(
        n_classes_dep_var=n_classes_dep,
        n_ind_vars=n_ind,
        n_categorical_vars=n_categorical,
    ...
    data = data_generator.generate_data()
    # Perform your processing on 'data' here

    model_count = 0
    total_models = 15120 #Need a way to calculate that
    iteration = 0  # Also needs to be passed as argument
    ...
    for link_function in ['logit', 'probit']:
        ...
        print("Logistic: Current Time =", current_time)
    ...
    for criterion, min_samples_leaf_percent, min_samples_split_percent in product(
        ...
        now = datetime.datetime.now()
        current_time = now.strftime("%H:%M:%S")
        print("DecisionTree: Current Time =", current_time)
    ...
    for criterion, min_samples_leaf_percent, min_samples_split_percent in product(
        ...
        now = datetime.datetime.now()
        current_time = now.strftime("%H:%M:%S")
        print("RF: Current Time =", current_time)
    # ANN
    """
...
with multiprocessing.Pool(processes=2) as pool: 
    results = pool.starmap(process_data, param_combinations)
